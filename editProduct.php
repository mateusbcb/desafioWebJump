<?php
  ...
  $findProduto = $produtos->select($id);
?>
...
<!-- Header -->
  ...
  <main class="content">
    ...
    <form action="routes/atualizarProduto.php" method="POST" multiple>
      <input type="hidden" name="id" value="<?php echo $findProduto[0]['id'] ?>">
      <div class="input-field">
        <label for="sku" class="label">Product SKU</label>
        <input type="text" id="sku" name="sku" value="<?php echo $findProduto[0]['sku'] ?>" class="input-text" /> 
      </div>
      <div class="input-field">
        <label for="name" class="label">Product Name</label>
        <input type="text" id="name" name="nome" value="<?php echo $findProduto[0]['nome'] ?>" class="input-text" /> 
      </div>
      <div class="input-field">
        <label for="price" class="label">Price</label>
        <input type="text" id="price" name="preco" value="<?php echo $findProduto[0]['preco'] ?>" class="input-text" /> 
      </div>
      <div class="input-field">
        <label for="quantity" class="label">Quantity</label>
        <input type="text" id="quantity" name="quantidade" value="<?php echo $findProduto[0]['quantidade'] ?>" class="input-text" /> 
      </div>
      <div class="input-field">
        ...
        <select multiple id="category" name="categorias[]" class="input-text">
          <?php
            foreach ($categoriasArr as $key => $categoria) {
              foreach (json_decode($findProduto[0]['categorias']) as $key => $categoriaSelected) {
                if ($categoria['nome'] == $categoriaSelected) {
      ...
      <div class="input-field">
        ...
        <textarea id="description" name="descricao" class="input-text"><?php echo $findProduto[0]['descricao'] ?></textarea>
      </div>
